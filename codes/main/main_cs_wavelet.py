...
if __name__ == "__main__":
    ...
    # パラメータの設定
    n_iter = 10000  # 反復回数
    alpha = 0.5  # 正則化パラメータ(lambda)
    LEVEL = 6  # ウェーブレット変換のレベル
    MOTHER_WAVELET = "db1"  # ウェーブレット変換のマザーウェーブレット

    # 圧縮センシングの適用
    recon = []
    for num in range(len(deficits)):  # すべての欠損値に対して適用
        Y_input = Y_masked_mean[num]
        recon_temp, _, _ = cs.cs_wavelet(Y_masked_mean[num], mask[num],  Y, n_iter, alpha, LEVEL, MOTHER_WAVELET="db1")
    ...
    plt.tight_layout()
    plt.savefig('dem_002_w_nonr_01/DEM_CS_5-20.png', dpi=220)
    plt.clf()
    ...
    # ax[1].set_title('interpolate with mean')
    plt.tight_layout()
    plt.savefig('dem_002_w_nonr_01/DEM_CS_25-40.png', dpi=220)
    plt.clf()
    ...
    # ax[1].set_title('interpolate with mean')
    plt.tight_layout()
    plt.savefig('dem_002_w_nonr_01/DEM_CS_50-70.png', dpi=220)
    plt.clf()
    ...
    # # ax[1].set_title('interpolate with mean')
    # plt.tight_layout()
    # plt.savefig('dem_002_w_nonr_01/DEM_CS_75-90.png', dpi=220)
    # plt.clf()
