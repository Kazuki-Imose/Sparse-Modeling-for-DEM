...
if __name__ == "__main__":
    ...
    files = glob.glob('E:/21_Sparse_Coding/files/targetdem_001.txt')
    # DEM の読み込み
    ...
    """ パッチサイズの決定 """
    # kernel_widths = [6, 10, 14, 22, 30, 38, 46, 54]
    # patch_size_cands = [8, 12, 16, 24, 32, 30, 48, 56]
    # patch_sizes = []  # mask された DEM の格納されるリスト
    # for i in range(len(deficits)):
    #     for j in range(len(kernel_widths)):
    #         kernel = np.ones((kernel_widths[j], kernel_widths[j]), np.uint8)  # カーネルの設定
    #         opening_mask = cv2.morphologyEx(mask[i], cv2.MORPH_CLOSE, kernel)
    #         num1 = im_row * im_col - np.sum(opening_mask)
    #         if num1 < 1:
    #             index = j
    #             break
    #     patch_size_temp = patch_size_cands[index]
    #     patch_sizes.append(patch_size_temp)
    # print(patch_sizes)
    ...
    dict_size = 48  # 要変更
    # 辞書の読み込み
    # A_DCT = np.fromfile('files/A_DCT_16_32').reshape((patch_size ** 2, dict_size ** 2))
    # 辞書の作成
    ...
    A_DCT.tofile('files/A_DCT_24_48')
    # 辞書の画像保存
    dl.show_dictionary(A_DCT, name='files/A_DCT_24_48.png')
    ...
    q_DCT = []
    for i in range(len(deficits)):
        q_DCT_temp = dl.sparse_coding_by_ElasticNet_with_mask(Y_masked[i], A_DCT, mask=mask[i], alpha=0.1, l1_ratio=0.5,
                                                                patch_size=patch_size)
        q_DCT.append(q_DCT_temp)
        q_DCT[i].tofile('dem_001_ela_nonr_01/DEM_001_q_' + str(deficits[i]) + 'ela')
        print(str(i + 1), '/', len(deficits), ' of sparse coding finished')
        # q_DCT_temp = np.fromfile('dem_001_ela_nonr_01/DEM_001_q_' + str(deficits[i]) + 'ela').reshape((-1, dict_size ** 2 + 1))
        # q_DCT.append(q_DCT_temp)
    ...
    Y_recon_DCT = []
    for i in range(len(deficits)):
        Y_recon_DCT_temp = dl.recon_image_by_ElasticNet(Y_masked[i], q_DCT[i], A_DCT, lam=0, patch_size=patch_size)
        Y_recon_DCT.append(Y_recon_DCT_temp)
        Y_recon_DCT[i].tofile('dem_001_ela_nonr_01/dem_recon_Y_' + str(deficits[i]) + '_ela')
        print(str(i + 1), '/', len(deficits), ' of image reconstruction finished')
        # Y_recon_DCT_temp = np.fromfile('dem_001_ela_nonr_01/dem_recon_Y_' + str(deficits[i]) + '_ela').reshape(im.shape)
        # Y_recon_DCT.append(Y_recon_DCT_temp)
    ...
    # ax[1].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[0])))
    # ax[2].set_title('deficit 10%')
    # ax[3].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[1])))
    # ax[4].set_title('deficit 20%')
    # ax[5].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[2])))
    # plt.tight_layout()
    # plt.savefig('dem_001_ela_nonr_01/hosei_nashi/dem_recon_DCT_5-20.png', dpi=220)
    # plt.clf()

    fig2, ax2 = plt.subplots(3, 2, figsize=(8, 12))
    ax2 = ax2.flatten()
    ax2[0].imshow(Y_masked[0], cmap='gray', interpolation='Nearest')
    ax2[1].imshow(Y_recon_DCT[0], cmap='gray', interpolation='Nearest')
    # ax2[2].imshow(Y_masked[1], cmap='gray', interpolation='Nearest')
    # ax2[3].imshow(Y_recon_DCT[1], cmap='gray', interpolation='Nearest')
    ...
    ax2[1].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[0])))
    # ax2[2].set_title('deficit 30%')
    # ax2[3].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[4])))
    # ax2[4].set_title('deficit 40%')
    # ax2[5].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[5])))
    plt.tight_layout()
    plt.savefig('dem_001_ela_nonr_01/hosei_nashi/dem_recon_DCT_60.png', dpi=220)
    plt.clf()
    ...
    # ax3[1].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[6])))
    # ax3[2].set_title('deficit 60%')
    # ax3[3].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[7])))
    # ax3[4].set_title('deficit 70%')
    # ax3[5].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[8])))
    # plt.tight_layout()
    # plt.savefig('dem_001_ela_nonr_01/hosei_nashi/dem_recon_DCT_50-70.png', dpi=220)
    # plt.clf()
    #
    ...
    # ax4[1].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[9])))
    # ax4[2].set_title('deficit 80%')
    # ax4[3].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[10])))
    # ax4[4].set_title('deficit 90%')
    # ax4[5].set_title('reconstructed by DCT with ElasticNet\n{:.6f}'.format(dl.get_rmse(im_zero, Y_recon_DCT[11])))
    # plt.tight_layout()
    # plt.savefig('dem_001_ela_nonr_01/hosei_nashi/dem_recon_DCT_75-90.png', dpi=220)
    # plt.clf()
    ...
    # plt.tight_layout()
    # plt.savefig('dem_001_ela_nonr_01/hosei_ari/dem_recon_DCT_5-20_1.png', dpi=220)
    # plt.clf()

    fig6, ax2 = plt.subplots(3, 2, figsize=(8, 12))
    ax2 = ax2.flatten()
    ax2[0].imshow(Y_masked[0], cmap='gray', interpolation='Nearest')
    ax2[1].imshow(Y_recon_DCT_1[0], cmap='gray', interpolation='Nearest')
    # ax2[2].imshow(Y_masked[1], cmap='gray', interpolation='Nearest')
    # ax2[3].imshow(Y_recon_DCT_1[1], cmap='gray', interpolation='Nearest')
    # ax2[4].imshow(Y_masked[5], cmap='gray', interpolation='Nearest')
    ...
    plt.tight_layout()
    plt.savefig('dem_001_ela_nonr_01/hosei_ari/dem_recon_DCT_60_1.png', dpi=220)
    plt.clf()
    ...
    # plt.tight_layout()
    # plt.savefig('dem_001_ela_nonr_01/hosei_ari/dem_recon_DCT_50-70_1.png', dpi=220)
    # plt.clf()
    #
    ...
    # plt.tight_layout()
    # plt.savefig('dem_001_ela_nonr_01/hosei_ari/dem_recon_DCT_75-90_1.png', dpi=220)
    # plt.clf()
